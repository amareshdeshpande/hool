(function () {
  ...
  var memberId=0;
  app.controller("createTableCtrl", function ($scope,$stomp, $rootScope,$localStorage,$timeout,$window,commonServ,$http,$location,tableServ) {
		
  		$rootScope.pageNo=2;
		
    	
    	//$rootScope.menuItems[0]=false;
				...
				tableServ.createTable($rootScope.table).then(function(result){					
					...
					memberId=memberInfo.memberId;				   
					 ...
					 tableServ.joinTableAsKibitzer(tableInfo).then(function(result){
						 ...
						 $stomp.send('/app/chat.sendMessage', {sender: 'saroj', type: 'UGTL'}, {});						
					 //	$scope.changePage('views/join_after_table.html',3);	
						 $scope.changePage('views/game_table.html',3);						
					 });
				});				
...
//============================================================================================================
	...
	app.controller("joinTableCtrl",function($scope,$rootScope,commonServ,$stomp,webapiUrl,$timeout,tableServ,$window,$localStorage,ngNotify){
	 		 $rootScope.pageNo=2;	
	 		 commonServ.resetMenu();    	
	    	 commonServ.setSideBar(2);	
				...
				if(table.noOfPlayer<4){
					...
					if(!($scope.tableListClicked)){
						...
						tableServ.joinTableAsKibitzer(tableInfo).then(function(result){				
								...
								$localStorage.table=JSON.stringify(table);						
								$scope.changePage('views/game_table.html',3);
								$stomp.send('/app/chat.sendMessage', {sender: memberId, type: 'UGTL'}, {});
								//$scope.changePage('views/join_after_table.html',3);
					...
					ngNotify.set("Table full!",{type: 'grimace',duration: 3000,position: 'top'});	
...
app.controller("gameTableCtrl", function ($scope, $rootScope,commonServ,$stomp,$log,webapiUrl,tableServ,$timeout,$localStorage,$sce,$location,ngNotify) {
	$rootScope.pageNo=3;
	$scope.isJoinTable=0;
	...
	//$scope.bottomCards=["8S","4S","3S","2S","AH","KH","JH","9H","8H","7H","AD","4D","AC"];
	...
	$scope.init=function(){
		...
		//commonServ.socketConn()
		$rootScope.connectWebSocket.then(function (frame) {
			
			subscription=$stomp.subscribe('/topic/table/'+table.id, function (payload, headers, res) {				
					...
					if(payload.type=='UGTL'){	
						...
						//To noify all user
						ngNotify.config({html: false});
						ngNotify.set(payload.content+ " left the table!",{type: 'grimace',duration: 3000,position: 'top'});
						$timeout(function(){
						...
						resetSharingInfo();
						$timeout(function(){
							var data1=JSON.parse(payload.content);
							...
							if(dealerPole==-1){
								...
								dealerPole=dealerPole==3?0:dealerPole+1;
							}*/
						...
						$timeout(function(){
							...
							//console.log('loggedInMemberPole :'+loggedInMemberPole+' pole '+data.pole);							
	...
	$scope.joinTableAsPlayer=function(val){
		...
		if((($scope.topMember.status==0) &&(val==0))||(($scope.rightMember.status==0)&&(val==1))
			...
			joinTableAsPlayer1(val);
		}

	}

	var joinTableAsPlayer1=function(val){
	//	alert('1421 :==='+val);
		var tableInfo={memberType:'KIBITZER', gameTableMemberKey : {tableId : table.id, memberId :memberId}};
		...
		tableServ.joinTableAsPlayer(playerInfo).then(function(result){
				...
				if(result.data.status='SUCCESS'){
					...
					var chatMessage = {sender: $localStorage.username, type: 'UGTL'};
					$stomp.send('/app/chat.sendMessage/'+table.id, chatMessage, {});

					// update game table list no of kibitzer and no of player
					$stomp.send('/app/chat.sendMessage', {sender: memberInfo.memberId, type: 'UGTL'}, {}); 
				...
				var palyerInfo={pole:value.pole, userName:value.member.userName,memberId:value.member.id,
					...
					if(count==4){
						...
						if(table.hostId==memberInfo.memberId){
							...
							$stomp.send('/app/chat.generateHand/'+table.id, {sender: memberInfo.memberId, type: 'DISTRIBUTE'}, {});	
							//$stomp.send('/app/chat.sendMessage/'+table.id, {sender: 'sarojkr', type: 'DISTRIBUTE'}, {});
						}
						//showPlayerArea(playersInfo);
					}
			});	
	   ...
	   if($scope.sevenCardArr[val]){
		   ...
		   //	shareRound+=1;
			   ...
			   var content=JSON.stringify({pole:$scope.bottomMember.pole,info,round:shareRound});
				   $stomp.send('/app/chat.sendMessage/'+table.id, {sender: memberInfo.memberId, type: 'SHARE',content:content}, {});
   ...
   //*======================================== Bidding Section==============================//
	...
	$scope.bidClick=function(){		
		...
		if(currentBid!==''){		
			...
			$stomp.send('/app/chat.sendMessage/'+table.id, {sender: memberInfo.memberId, type: 'BID',content:content}, {});
			$scope.biddingStart=false;	
			currentSuitIndex=-1;
			currentSuitNum=-1;
			currentBid='';
		...
		if($scope.biddingRound==1&&count==4){
			...
			$scope.activePole=[false,false,false,false];
				...
				if(passCount==2){
					...
					if(table.hostId==memberInfo.memberId){							
						$timeout(function(){						
							$stomp.send('/app/chat.sendMessage/'+table.id, {sender: memberInfo.memberId, type: 'START_PLAY',content:content}, {});
						},500);	
					}					
			...
			/*	if(bid.substring(1)=='C'){
				...
				*/
				...
				if($scope.topMember.pole==bidPole){
					$scope.topMember.bidStatus=true;
					$scope.topMember.bid={suit:suit,num:suitcount};
					//$scope.topMember.bid={suit:bid.substring(1),num:suitcount};
					
				}else if($scope.leftMember.pole==bidPole){
					$scope.leftMember.bidStatus=true;
					$scope.leftMember.bid={suit:suit,num:suitcount};
				//	$scope.leftMember.bid={suit:bid.substring(1),num:suitcount};
					
				}else if($scope.rightMember.pole==bidPole){
					$scope.rightMember.bidStatus=true;
					$scope.rightMember.bid={suit:suit,num:suitcount};
					//$scope.rightMember.bid={suit:bid.substring(1),num:suitcount};
	...
	var setSuitNumber=function(roundNum){	
		...
		/*if((roundNum==3)&&(loggedInMemberPole==$scope.tempDeclarerPartnerPole)){
			highestNum=7;
		}
		*/
...
*/		
	...
	$scope.playCard=function(card,type){	
        ...
        if($scope.isPoleActiveToPlay[pole] && loggedInMemberPole!=$scope.dummy.pole){
			if(playedCardInfo.length==0 || checkForStartingSuitCard(cardSuit,pole,type)==true){
				...
				$stomp.send('/app/chat.sendMessage/'+table.id, {sender: memberInfo.memberId, type: 'PLAY_CARD', content:content}, {});
			...
			for(var i=0;i<playedCardInfo.length; i++){
				...
				if(i==3){	
					...
					//var transImg="assets/images/deck/trans.svg";
						...
						if(wonTricksCount[0]+wonTricksCount[1]==13){
						...
						// alert(JSON.stringify(dealInfo));
						    ...
						    if(loggedInMemberPole==dealerPole){	
								tableServ.saveGameResult(dealInfo).then(function(result){				
									 $stomp.send('/app/chat.sendMessage/'+table.id, {sender: memberInfo.memberId, type: 'SCORE', content:JSON.stringify(result)}, {});
	...
	var showAllCards=function(){		
		...
		//	console.log(JSON.stringify(hands_1));
	...
	var showGameResult=function(result){
		  showAllCards();
		  $timeout(function(){
			  $scope.NSScore=result.data.NSPoint;
			  $scope.EWScore=result.data.EWPoint;
	...
	*/
	...
	$scope.$on('$destroy', function() {	
		...
		tableServ.leaveTable(tableInfo).then(function(result){			
			$stomp.send('/app/chat.sendMessage', {sender: memberInfo.memberId,content:memberInfo.loginId, type: 'UGTL'}, {});
		    $stomp.send('/app/chat.sendMessage/'+table.id, {sender: memberInfo.memberId,content:memberInfo.loginId, type: 'LEAVE'}, {});
		});
		// $localStorage.$reset();
	})
});
